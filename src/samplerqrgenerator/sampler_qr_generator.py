...
def process_xml(
    ...
    for run in field_entry:
        ...
        for sample in run.findall("Sample"):
            ...
            sample_root.attrib = {"ID": sample_id}
            ...
            xml_string = ET.tostring(
                sample_root, encoding="utf8", method="xml"
            ).decode()
...
def generate_qr_code_from_string(payload: str) -> Image.Image:
    ...
    """
    qr = qrcode.QRCode(
        version=9, error_correction=qrcode.constants.ERROR_CORRECT_L, border=8
    )
...
def create_printable_a4_page(
    qr_images: List[Image.Image], data_dicts: List[Dict[str, str]], output_filename: str
) -> None:
    """
    ...
    text_block_length = (0.85 * cell_width) // font_size
    ...
    for i, qr_image in enumerate(qr_images):
        ...
        a4_page.paste(
            qr_image_resized,
            (
                margins + cell_buffer + (col * cell_width),
        ...
        draw.text((text_pos_x, text_pos_y), final_text, fill=(0, 0, 0), font=font)
    ...
    a4_page.convert("RGB").save(f"{output_filename}.pdf", "PDF", resolution=300)


def create_printable_label_document(
    qr_images: List[Image.Image],
    data_dicts: List[Dict[str, str]],
    output_filename: str,
    ...
    logging.debug(
        f"Compiling label with dimensions px dimensions {label_width}, {label_height} at 300 pdi"
    )
    ...
    text_block_length = int((0.85 * label_width) // font_size)
    ...
    for site, qr_image in enumerate(qr_images):
        ...
        qr_image_resized = qr_image.resize(
            (qr_width, qr_height), resample=Image.LANCZOS
        )

        label_page.paste(qr_image_resized, (qr_buffer, qr_buffer))
        label_page.paste(
            logo_image_resized,
            (logo_x - logo_buffer, logo_y - logo_buffer),
            mask=logo_image_resized,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        dest="logging_level",
        default="INFO",
        help="Show only critical log messages",
    )
    parser.add_argument(
        "-c",
        "--concise",
    ...
    parser.add_argument(
        "--logfile", type=str, help="Indicate a file to which to save the logs"
    )
